...
class DatabaseHandler:
    ...
    async def reduce_points(self, user_id: int, amount: int) -> int:
        ...
        return new_balance
    #Process Claim
    async def process_daily_claim(self, user_id, daily_amount: int, interest_rate: float, cooldown: int) -> tuple[bool, int, int, int, int]:
        """
        Returns
        -------
    ...
    async def transfer_point(self, src_user_id: int, end_user_id: int, amount:int) -> tuple[bool,int]:
        try:
            async with self.pool.acquire() as conn:
                conn: acpg.Connection
